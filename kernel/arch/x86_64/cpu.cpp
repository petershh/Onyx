...
static void __cpu_identify(void)
{
    ...
    if (__get_cpuid(0x15, &eax, &ebx, &ecx, &edx))
    {
        ...
        else if (ecx == 0)
        {
            ...
            {
                cpu.apic_rate = (uint64_t) eax * 1000 * denominator / numerator;
                cpu.tsc_rate = (uint64_t) eax * 1000000;
        ...
        {
            cpu.apic_rate = ecx / 1000;
            cpu.tsc_rate = (uint64_t) ecx * ebx / eax;
            pr_info("x86cpu: CPUID reported TSC rate: %lu, apic rate: %lu\n", cpu.tsc_rate,
                    cpu.apic_rate);
        }
    }
